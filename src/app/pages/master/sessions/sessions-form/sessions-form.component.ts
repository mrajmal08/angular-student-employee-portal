  ...
  onSubmit() {
    if (this.agentForm.valid) {
      if (!!this.dataForEdit.row) {
        this.apiClient
          .post(
            `session/update?id=${this.dataForEdit.row.id}&name=${this.agentForm.controls['name'].value}&description=${this.agentForm.controls['description'].value}`
          )
          .subscribe((resp: any) => {
            if (resp.status) {
              this.location.back();
        ...
        this.apiClient
          .post(
            `session/insert?name=${this.agentForm.controls['name'].value}&description=${this.agentForm.controls['description'].value}`
          )
          .subscribe((resp: any) => {
            if (resp.status) {
              this.location.back();
